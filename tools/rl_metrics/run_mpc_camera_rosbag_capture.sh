...
REALTIME_PERSONA_STYLE="${REALTIME_PERSONA_STYLE:-passenger_casual_light}"
REALTIME_CHARACTER_PROFILE_FILE="${REALTIME_CHARACTER_PROFILE_FILE:-/repo_tools/aituber_passenger_demo/aituber_character_profile.json}"
...
REALTIME_PERSONA_STYLE=${REALTIME_PERSONA_STYLE}
REALTIME_CHARACTER_PROFILE_FILE=${REALTIME_CHARACTER_PROFILE_FILE}
...
if [[ "${REALTIME_COMMENTARY}" == "true" && "${REALTIME_COMMENTARY_AFTER_CAPTURE}" != "true" ]]; then
  ...
  docker compose run -d --name "${RUN_ID}_realtime_commentary" -v "${ROOT}/tools:/repo_tools:ro" --entrypoint bash autoware-command -lc "
      ...
      exec python3 /repo_tools/rl_metrics/realtime_vlm_commentary_node.py \
        ...
        --persona-style '${REALTIME_PERSONA_STYLE}' \
        --character-profile-file '${REALTIME_CHARACTER_PROFILE_FILE}' \
