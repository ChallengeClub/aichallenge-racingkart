"""Small longitudinal controller used by the End-to-End PilotNet node."""


class SpeedController:
    """Convert wheel-odometry speed error into a bounded acceleration command."""
    ...
    def compute(self, current_speed_mps: float, target_speed_mps: float | None = None) -> float:
        speed = max(0.0, float(current_speed_mps))
        target = self.target_speed_mps if target_speed_mps is None else max(
            0.0, float(target_speed_mps)
        )
        acceleration = self.proportional_gain * (target - speed)
        return max(self.min_acceleration, min(self.max_acceleration, acceleration))
