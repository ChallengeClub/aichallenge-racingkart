...
結果にはlapの遷移時刻、遷移間から求めた周回時間、最短周回時間も保存されます。ランダムな開始位置から最初のスタートラインまでを周回時間へ含めずに比較できます。ただしNPCありでは`/awsim/status`に複数車両の値が混ざる可能性があるため、厳密なタイム比較にはNPCなしの診断走行を使い、NPCありの値は継続走行の判定と参考値に使います。
